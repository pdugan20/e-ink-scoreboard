...
echo "----------------------"
tail -n 1000 "$LOG_FILE" | grep -i "screenshot\|playwright\|webkit" | tail -n 20 || echo "No screenshot activity found"
echo ""
...
if command -v ps &> /dev/null; then
    ...
    ps aux | grep -E "eink_display|sports-|webkit|playwright" | grep -v grep || echo "No related processes found"
    echo ""
fi
...
echo "-------------------"
if command -v ps &> /dev/null; then
    ps aux | grep -E "webkit|playwright" | grep -v grep | head -n 10 || echo "No browser processes found"
fi
