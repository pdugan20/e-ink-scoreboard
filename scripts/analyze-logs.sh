...
if [[ ! -f "$LOG_FILE" ]]; then
    ...
    echo "Note: Make sure the display service is running and has write permissions to ~/logs/"
    exit 1
fi
...
echo "Note: For real-time monitoring: tail -f $LOG_FILE | grep -E 'ERROR|CRITICAL|RESOURCE_SNAPSHOT|\[WORKER\]'"
